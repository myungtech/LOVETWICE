...
<link type="text/css" rel="stylesheet"href="../../resources/css/brd_css/tnotice_detail.css">
...
<script type="text/javascript">
	...
	function fn_openBoardList() {
		window.location.href = "/twentyOnce/brdviews/not_Notice/list?curPage=" + "${map.curPage}"
...
// 	<입력>
	 ...
	 if ($("#reply_content").val().length < 1) {// 댓글 내용항목이 없음
				alert("댓글 내용 없음");
...
<body>
	...
	<form action="content_modify" method="post">
		...
		<input type="hidden" name="not_title" value="${content_view.not_title }" /> <input type="hidden"
			...
			<tfoot>
				<tr>
					...
					<input type="submit" id="btnmodify"	value="modify" /> &nbsp;&nbsp;
						<a href="delete?num=${content_view.not_num }">DELETE</a>&nbsp;&nbsp;
				<a	href="download?p=resources/upload/
			      &f=${content_view.bfilesrc}
			       &bid=${content_view.not_num}">${content_view.bfilesrc }</a>
			    <br>
			    
						<%} %>
						<a href="javascript:fn_openBoardList()">LIST</a> </td>
		...
		<tbody>
			<tr>
				...
				<td><textarea id="reply_content" name="not_rcontent" cols="85"
						rows="5" placeholder="댓글을 입력하시오"></textarea></td>
